...
// extension.                       --- save previous words used and cross check if random word has been used before.
...
function GenerateGrid(){
    // should delete current grid. if grid is already there.
...
function addKeyEventListeners(){
    ...
    for (key of keys){
        //console.log(key)
        key.style.backgroundcolor="light gray";
...
function restartScreen(result){
    //document.getElementById('pleaseSubmit').remove()
...
function handleDelete(){
    ...
    let currentX=0
    let currentY=0
    //console.log(`${nextY-1},${previousSubmittedRow}`)
    //console.log((nextY-1)==previousSubmittedRow)
...
// ---------------------- make it call restart function which handles playing again
                for(let i=0;i<word.length;i++){
                document.getElementById(`${i},5`).style.backgroundColor="green";
                }
                win =true;
                restartScreen(win)
                return // make a won function
            }
        else{
            win=false;
            restartScreen(win)
            return // make a  lost function 
        }}
    let nextAvailableRow = nextGridSpace[1]
    //let nextAvailableRow = nextGridSpace[1] // if undefined then lost or 
    // if (nextAvailableRow===''){
    //     if(word==targetWord.toLocaleLowerCase()){
    ...
    else if (nextAvailableRow==1){ // for the first row
        currentRow=0;
    }
    else{
    ...
    ///alert(notInWordList(word))x
    if(!notInWordList(word)){
        notInWordListMessage() 
        return
    }
    if(word==targetWord.toLocaleLowerCase()){
  // ---------------------- make it call restart function which handles playing again
...
//    //alert(word)
    ...
    // get all 5 letters from current row
    // compare with target word 
    // style grid and keyboard
    // allow win if is correct
...
function handleClick(event){
    ...
    if (needSubmit){
        ...
        //     document.getElementById('notWord').remove()
        // }
        let pleaseSubmit =document.createElement('h4')
...
//styling if letter is in the correct column and placement in word. also styling incase it is in word but not in corret location.

// also styling keyboard for if letter has been pressed but not in word.

// also a checker that forces user to only choose a correct english word

// possible extension. save wins and looses data. offer option to play again. save words that have been used so that user can't play with them again.
...
// images for delete and submit for